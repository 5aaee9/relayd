...
pub const Address = extern union {
    ...
    pub const ListenError = compat.PosixIoError ||
    ...
    pub fn listen(address: Address, options: ListenOptions) ListenError!Server {
        ...
        const sockfd = try compat.socket(address.any.family, sock_flags, proto);
        var s: Server = .{
        ...
        try compat.bind(sockfd, &address.any, socklen);
        try compat.listen(sockfd, options.kernel_backlog);
        try compat.getsockname(sockfd, &s.listen_address.any, &socklen);
        return s;
    }
};
...
pub fn connectUnixSocket(path: []const u8) !Stream {
    ...
    const sockfd = try compat.socket(
        posix.AF.UNIX,
    ...
    try compat.connect(sockfd, &addr.any, addr.getOsSockLen());
...
fn if_nametoindex(name: []const u8) IPv6InterfaceError!u32 {
    if (native_os == .linux) {
        ...
        const sockfd = try compat.socket(posix.AF.UNIX, posix.SOCK.DGRAM | posix.SOCK.CLOEXEC, 0);
...
pub const TcpConnectToAddressError = compat.PosixIoError;
...
pub fn tcpConnectToAddress(address: Address) TcpConnectToAddressError!Stream {
    ...
    const sockfd = try compat.socket(address.any.family, sock_flags, posix.IPPROTO.TCP);
    errdefer Stream.close(.{ .handle = sockfd });

    try compat.connect(sockfd, &address.any, address.getOsSockLen());
...
fn linuxLookupName(
    ...
    for (addrs.items, 0..) |*addr, i| {
        ...
        if (compat.socket(addr.addr.any.family, sock_flags, posix.IPPROTO.UDP)) |fd| syscalls: {
            defer Stream.close(.{ .handle = fd });
            compat.connect(fd, da, dalen) catch break :syscalls;
            key |= DAS_USABLE;
            compat.getsockname(fd, sa, &salen) catch break :syscalls;
...
const ResolvConf = struct {
    ...
    fn resMSendRc(
        ...
        const fd = compat.socket(family, flags, 0) catch |err| switch (err) {
            error.AddressFamilyNotSupported => blk: {
                // Handle case where system lacks IPv6 support
                if (family == posix.AF.INET6) {
                    family = posix.AF.INET;
                    break :blk try compat.socket(posix.AF.INET, flags, 0);
                }
                return err;
            },
            else => |e| return e,
        ...
        var next: u32 = 0;
        var t2: u64 = @bitCast(compat.milliTimestamp());
        const t0 = t2;
        var t1 = t2 - retry_interval;

        var servfail_retry: usize = undefined;

        outer: while (t2 - t0 < timeout) : (t2 = @as(u64, @bitCast(compat.milliTimestamp()))) {
            if (t2 - t1 >= retry_interval) {
                ...
                while (i < queries.len) : (i += 1) {
                    if (answers[i].len == 0) {
                        for (ns_list.items) |*ns| {
                            _ = compat.sendto(fd, queries[i], posix.MSG.NOSIGNAL, &ns.any, sl) catch undefined;
                        }
                    }
                }
                t1 = t2;
            ...
            while (true) {
                var sl_copy = sl;
                const rlen = compat.recvfrom(fd, answer_bufs[next], 0, &sa.any, &sl_copy) catch break;
...
pub const Stream = struct {
    ...
    pub const WriteError = compat.PosixIoError || error{
        ConnectionResetByPeer,
        SocketNotBound,
        MessageTooBig,
        NetworkSubsystemFailed,
            ...
            pub fn init(net_stream: Stream, buffer: []u8) Reader {
                return .{
                    .file_reader = .{
                        ...
                        .file = .{ .handle = net_stream.handle, .flags = .{ .nonblocking = false } },
                        .mode = .streaming,
        ...
        .windows => struct {
            ...
            pub fn init(stream: Stream, buffer: []u8) Writer {
                return .{
                    ...
                    .file_writer = .initStreaming(.{ .handle = stream.handle, .flags = .{ .nonblocking = false } }, compat.io(), buffer),
                };
            }
            ...
            fn drain(io_w: *Io.Writer, data: []const []const u8, splat: usize) Io.Writer.Error!usize {
                ...
                return io_w.consume(compat.sendmsg(w.file_writer.file.handle, &msg, flags) catch |err| {
                    w.err = err;
                    return error.WriteFailed;
                });
            }
    ...
    pub fn writev(self: Stream, iovecs: []const posix.iovec_const) WriteError!usize {
        return @errorCast(compat.writev(self.handle, iovecs));
    }
...
pub const Server = struct {
    ...
    pub const AcceptError = compat.PosixIoError;
    ...
    pub fn accept(s: *Server) AcceptError!Connection {
        ...
        const fd = try compat.accept(s.stream.handle, &accepted_addr.any, &addr_len, posix.SOCK.CLOEXEC);
        return .{
            .stream = .{ .handle = fd },
